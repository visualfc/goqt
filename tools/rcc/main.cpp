...
#include <rcc.h>
//#include <QtCore/../../src/corelib/kernel/qcorecmdlineargs_p.h>
...
void showHelp(const QString &argv0, const QString &error)
{
    ...
    fprintf(stderr, "Usage: %s  [options] <inputs>\n\n"
        ...
        "  -go pkg              write output to go source format with pkg\n"
    ...
    for (int i = 1; i < args.count() && errorMsg.isEmpty(); i++) {
        ...
        if (args[i][0] == QLatin1Char('-')) {   // option
            ...
            if (opt == QLatin1String("-o")) {
                ...
                outFilename = args[++i];
            } else if (opt == QLatin1String("-go")) {
                if (!(i < argc-1)) {
                    errorMsg = QLatin1String("Missing go package name");
                    break;
                }
                library.setGoPkg(args[++i]);
